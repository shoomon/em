...
export const fetchYoutubeSearch = async (query: string) => {
  ...
  const url = `${origin}/youtube/v3/search?part=id&regionCode=kr&type=video&videoCategoryId=10&topicId=/m/04rlf&videoEmbeddable=true&videoLicense=youtube&safeSearch=strict&key=${API_KEY}&maxResults=1&q=${query}`
